package cpsc2150.extendedConnectX;
import java.util.Vector;
/**
 * The controller class will handle communication between our View and our Model (IGameBoard)
 * <p>
 * This is where you will write code
 * <p>
 * You will need to include your IGameBoard interface
 * and both of the IGameBoard implementations from Project 4
 * If your code was correct you will not need to make any changes to your IGameBoard implementation class
 */
public class ConnectXController {

    //our current game that is being played
    private IGameBoard curGame;

    //The screen that provides our view
    private ConnectXView screen;

    public static final int MAX_PLAYERS = 10;
    //our play tokens are hard coded. We could make a screen to get those from the user, but

    private int numPlayers;
    private char[] playerSymbols = new char[] {'X', 'O', 'Y', 'Z', 'A', 'B', 'C', 'M', 'J', 'K'};
    private char currentPlayer;
    private int row = 0;
    private boolean gameMode;
    /**
     * @param model the board implementation
     * @param view  the screen that is shown
     * @post the controller will respond to actions on the view using the model.
     */
    ConnectXController(IGameBoard model, ConnectXView view, int np) {
        this.curGame = model;
        this.screen = view;
        numPlayers = np;
    }

    /**
     * @param col the column of the activated button
     * @post will allow the player to place a token in the column if it is not full, otherwise it will display an error
     * and allow them to pick again. Will check for a win as well. If a player wins it will allow for them to play another
     * game hitting any button
     */
    public void processButtonClick(int col)
    {
        if(gameMode == true)
        {
            newGame();
        }
        else {

            for (int i = 0; i < curGame.getNumRows(); i++)
            {
                //find empty row
               if (curGame.whatsAtPos(new BoardPosition(i, col)) == ' ')
               {
                   row = i;
                   break;
               }
            }
            //see if column is full
            if (!curGame.checkIfFree(col))
            {
                screen.setMessage("This column is full. It is still "+ playerSymbols[currentPlayer] + "'s turn.");
                return;
            }
            //place token
           curGame.placeToken(playerSymbols[currentPlayer], col);
           BoardPosition pos = new BoardPosition(row, col);
           screen.setMarker(pos.getRow(), col, playerSymbols[currentPlayer]);
           //check for tie
            if(curGame.checkTie())
            {
                //check if last token placed was a win
                if(curGame.checkForWin(col))
                {
                    screen.setMessage("Player " + playerSymbols[currentPlayer] + " has won! To play again, press any key.");
                    gameMode = true;
                }
                screen.setMessage("It's a tie! To play again, press any key.");
                gameMode = true;
            }
            //check for win
           else if(curGame.checkForWin(col))
           {
               screen.setMessage("Player " + playerSymbols[currentPlayer] + " has won! To play again, press any key.");
               gameMode = true;
           }

           else {
               //change player
               currentPlayer++;
               if(numPlayers <= currentPlayer)
               {
                   currentPlayer = 0;
               }
               screen.setMessage("It's " + playerSymbols[currentPlayer] + "'s turn.");
           }

        }


    }

    /**
     * This method will start a new game by returning to the setup screen and controller
     */
    private void newGame() {
        //close the current screen
        screen.dispose();
        //start back at the set up menu
        SetupView screen = new SetupView();
        SetupController controller = new SetupController(screen);
        screen.registerObserver(controller);
    }
}